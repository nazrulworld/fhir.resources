...
def test_AllergyIntolerance_1(base_settings):
    filename = base_settings["unittest_data_dir"] / "allergyintolerance-example-refuted.canonical.json"
...
def impl_AllergyIntolerance_1(inst):
    ...
    assert inst.extension[0].url == "http://hl7.org/fhir/StructureDefinition/allergyintolerance-reasonRefuted"
    assert inst.extension[0].valueCodeableConcept.coding[0].code == "MED"
    assert inst.extension[0].valueCodeableConcept.coding[0].display == "Medical Status Altered"
...
def test_AllergyIntolerance_2(base_settings):
    filename = base_settings["unittest_data_dir"] / "allergyintolerance-example.canonical.json"
    inst = allergyintolerance.AllergyIntolerance.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "AllergyIntolerance" == inst.resource_type
...
def impl_AllergyIntolerance_2(inst):
    ...
    assert inst.reaction[0].description == "Challenge Protocol. Severe Reaction to 1/8 cashew. Epinephrine administered"
    ...
    assert inst.reaction[0].substance.coding[0].system == "http://www.nlm.nih.gov/research/umls/rxnorm"
...
def test_AllergyIntolerance_3(base_settings):
    filename = base_settings["unittest_data_dir"] / "allergyintolerance-fishallergy.canonical.json"
    inst = allergyintolerance.AllergyIntolerance.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "AllergyIntolerance" == inst.resource_type
...
def test_AllergyIntolerance_4(base_settings):
    filename = base_settings["unittest_data_dir"] / "allergyintolerance-medication.canonical.json"
    inst = allergyintolerance.AllergyIntolerance.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "AllergyIntolerance" == inst.resource_type
...
def impl_AllergyIntolerance_4(inst):
    ...
    assert inst.substance.coding[0].system == "http://www.nlm.nih.gov/research/umls/rxnorm"
