...
def test_DetectedIssue_1(base_settings):
    filename = base_settings["unittest_data_dir"] / "detectedissue-example-allergy.canonical.json"
...
def test_DetectedIssue_2(base_settings):
    filename = base_settings["unittest_data_dir"] / "detectedissue-example-dup.canonical.json"
    inst = detectedissue.DetectedIssue.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "DetectedIssue" == inst.resource_type
...
def impl_DetectedIssue_2(inst):
    ...
    assert inst.implicated[1].display == "Image 1 from Series 3: CT Images on Patient MINT (MINT1234) taken at 1-Jan 2011 01:20 AM"
...
def test_DetectedIssue_3(base_settings):
    filename = base_settings["unittest_data_dir"] / "detectedissue-example-lab.canonical.json"
    inst = detectedissue.DetectedIssue.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "DetectedIssue" == inst.resource_type
...
def test_DetectedIssue_4(base_settings):
    filename = base_settings["unittest_data_dir"] / "detectedissue-example.canonical.json"
    inst = detectedissue.DetectedIssue.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "DetectedIssue" == inst.resource_type
...
def impl_DetectedIssue_4(inst):
    ...
    assert inst.mitigation[0].action.coding[0].system == "http://hl7.org/fhir/v3/ActCode"
