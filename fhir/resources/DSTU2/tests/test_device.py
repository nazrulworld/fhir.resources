...
def test_Device_1(base_settings):
    filename = base_settings["unittest_data_dir"] / "device-example-f001-feedingtube.canonical.json"
...
def impl_Device_1(inst):
    ...
    assert inst.manufactureDate == "2015-08-08"
...
def test_Device_2(base_settings):
    filename = base_settings["unittest_data_dir"] / "device-example-ihe-pcd.canonical.json"
    inst = device.Device.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Device" == inst.resource_type
...
def impl_Device_2(inst):
    ...
    assert inst.identifier[0].type.coding[0].system == "http://hl7.org/fhir/identifier-type"
...
def test_Device_3(base_settings):
    filename = base_settings["unittest_data_dir"] / "device-example-pacemaker.canonical.json"
    inst = device.Device.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Device" == inst.resource_type
...
def impl_Device_3(inst):
    ...
    assert inst.type.coding[0].display == "Performance pace maker for high octane patients"
    assert inst.type.coding[0].system == "http://acme.com/devices"


def test_Device_4(base_settings):
    filename = base_settings["unittest_data_dir"] / "device-example-software.canonical.json"
    inst = device.Device.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Device" == inst.resource_type
...
def impl_Device_4(inst):
    ...
    assert inst.manufacturer == "Acme Devices, Inc"
    assert inst.text.div == "<div><p><b>Generated Narrative with Details</b></p><p><b>id</b>: software</p><p><b>identifier</b>: goodhealth</p><p><b>type</b>: EHR <span>(Details )</span></p><p><b>manufacturer</b>: Acme Devices, Inc</p><p><b>version</b>: 10.23-23423</p><p><b>contact</b>: http://acme.com</p><p><b>url</b>: <a>http://acme.com/goodhealth/ehr/</a></p></div>"
    assert inst.text.status == "generated"
...
def impl_Device_5(inst):
    ...
    assert inst.identifier[1].type.coding[0].system == "http://hl7.org/fhir/identifier-type"
    ...
    assert inst.note[0].time == "2015-06-28T14:03:32+10:00"
    assert inst.status == "available"
