...
def test_DeviceComponent_1(base_settings):
    filename = base_settings["unittest_data_dir"] / "devicecomponent-example-prodspec.canonical.json"
...
def impl_DeviceComponent_1(inst):
    ...
    assert inst.contained[0].identifier[0].type.coding[0].system == "http://hl7.org/fhir/identifier-type"
    ...
    assert inst.contained[0].identifier[1].type.text == "Global Medical Device Nomenclature"
    ...
    assert inst.lastSystemChange == datetime(2014, 10, 7, 14, 45, 0, tzinfo=timezone.utc)
    ...
    assert inst.productionSpecification[1].specType.coding[0].display == "Hardware version"
    assert inst.productionSpecification[2].productionSpec == "1.12"
    assert inst.productionSpecification[2].specType.coding[0].code == "4"
    assert inst.productionSpecification[2].specType.coding[0].display == "Software version"
    ...
    assert inst.productionSpecification[3].specType.coding[0].display == "Firmware version"
...
def test_DeviceComponent_2(base_settings):
    filename = base_settings["unittest_data_dir"] / "devicecomponent-example.canonical.json"
    inst = devicecomponent.DeviceComponent.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "DeviceComponent" == inst.resource_type
