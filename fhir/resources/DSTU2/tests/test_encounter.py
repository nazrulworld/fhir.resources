...
def test_Encounter_1(base_settings):
    filename = base_settings["unittest_data_dir"] / "encounter-example-f001-heart.canonical.json"
...
def test_Encounter_2(base_settings):
    filename = base_settings["unittest_data_dir"] / "encounter-example-f002-lung.canonical.json"
    inst = encounter.Encounter.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Encounter" == inst.resource_type
...
def test_Encounter_3(base_settings):
    filename = base_settings["unittest_data_dir"] / "encounter-example-f003-abscess.canonical.json"
    inst = encounter.Encounter.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Encounter" == inst.resource_type
...
def impl_Encounter_3(inst):
    ...
    assert inst.reason[0].coding[0].system == "http://snomed.info/sct"
    assert inst.reason[0].extension[0].url == "http://hl7.org/fhir/StructureDefinition/encounter-primaryDiagnosis"
...
def test_Encounter_4(base_settings):
    filename = base_settings["unittest_data_dir"] / "encounter-example-f201-20130404.canonical.json"
    inst = encounter.Encounter.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Encounter" == inst.resource_type
...
def impl_Encounter_4(inst):
    ...
    assert inst.priority.coding[0].system == "http://snomed.info/sct"
    assert inst.reason[0].text == "The patient had fever peaks over the last couple of days. He is worried about these peaks."
...
def test_Encounter_5(base_settings):
    filename = base_settings["unittest_data_dir"] / "encounter-example-f202-20130128.canonical.json"
    inst = encounter.Encounter.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Encounter" == inst.resource_type
...
def impl_Encounter_5(inst):
    ...
    assert inst.indication[0].extension[0].url == "http://hl7.org/fhir/StructureDefinition/encounter-primaryDiagnosis"
...
def test_Encounter_8(base_settings):
    filename = base_settings["unittest_data_dir"] / "encounter-example-xcda.canonical.json"
    inst = encounter.Encounter.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "Encounter" == inst.resource_type
...
def impl_Encounter_8(inst):
    ...
    assert inst.reason[0].coding[0].system == "http://ihe.net/xds/connectathon/eventCodes"
    assert inst.status == "finished"
...
def impl_Encounter_9(inst):
    ...
    assert inst.extension[0].extension[1].valueCodeableConcept.coding[0].code == "8319008"
    assert inst.extension[0].extension[1].valueCodeableConcept.coding[0].display == "Principal diagnosis"
    assert inst.extension[0].extension[1].valueCodeableConcept.coding[0].system == "http://snomed.info/sct"
    assert inst.extension[0].url == "http://hl7.org/fhir/StructureDefinition/encounter-relatedCondition"
    ...
    assert inst.hospitalization.dischargeDisposition.coding[0].system == "http://hl7.org/fhir/discharge-disposition"
