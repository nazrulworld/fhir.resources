...
class MediaTests(unittest.TestCase):
    ...
    def implMedia1(self, inst):
        ...
        self.assertEqual(
            inst.extension[0].url, "http://nema.org/fhir/extensions#0002-0010"
        )
        self.assertEqual(inst.extension[0].valueUri, "urn:oid:1.2.840.10008.1.2.1")
        self.assertEqual(inst.height, 480)
        self.assertEqual(
            inst.id, "1.2.840.11361907579238403408700.3.0.14.19970327150033"
        )
    ...
    def implMedia2(self, inst):
        ...
        self.assertEqual(
            inst.subtype.coding[0].system, "http://hl7.org/fhir/media-method"
        )
        self.assertEqual(
            inst.text.div,
            '<div>Diagram for Patient Henry Levin (MRN 12345):<br/><img alt="diagram" src="#11"/></div>',
        )
        self.assertEqual(inst.text.status, "generated")
        self.assertEqual(inst.type, "photo")
    ...
    def implMedia3(self, inst):
        ...
        self.assertEqual(
            inst.text.div,
            '<div>Sound recording of speech example for Patient Henry Levin (MRN 12345):<br/><img alt="diagram" src="#11"/></div>',
        )
        self.assertEqual(inst.text.status, "generated")
