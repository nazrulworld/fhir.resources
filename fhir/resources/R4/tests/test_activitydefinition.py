...
def impl_activitydefinition_4(inst):
    ...
    assert (
        inst.relatedArtifact[1].resource == "ActivityDefinition/serum-dengue-virus-igm"
    )
...
def impl_activitydefinition_9(inst):
    ...
    assert inst.status == "draft"
    assert inst.text.status == "generated"
    assert inst.transform == "StructureMap/supplyrequest-transform"
