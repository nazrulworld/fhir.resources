...
class FHIRAbstractBase(object):
    ...
    def with_json(cls, jsonobj):
        ...
        raise TypeError(
            "`with_json()` on {} only takes dict or list of dict, but you provided {}".format(
                cls, type(jsonobj)
            )
        )
    ...
    def _with_json_dict(cls, jsondict):
        ...
        if not isinstance(jsondict, dict):
            raise TypeError(
                "Can only use `_with_json_dict()` on {0} with a dictionary, but got {1}".format(
                    cls, type(jsondict)
                )
            )
        return cls(jsondict)
    ...
    def update_with_json(self, jsondict):
        ...
        if not isinstance(jsondict, dict):
            raise FHIRValidationError(
                "Non-dict type {} fed to `update_with_json` on {}".format(
        ...
        valid = set(
            ["resourceType"]
        )  # used to also contain `fhir_comments` until STU-3
        found = set()
        nonoptionals = set()
        for (
            name,
            ...
            if value is not None:
                ...
                if is_list:
                    if not isinstance(value, list):
                        err = TypeError(
                            'Wrong type {} for list property "{}" on {}, expecting a list of {}'.format(
                ...
                if testval is not None and not self._matches_type(testval, typ):
                    err = TypeError(
                        'Wrong type {} for property "{}" on {}, expecting {}'.format(
            ...
            if err is not None:
                errs.append(
                    err.prefixed(name)
                    if isinstance(err, FHIRValidationError)
                    else FHIRValidationError([err], name)
                )
        ...
        if len(nonoptionals) > 0:
            for miss in nonoptionals - found:
                errs.append(
                    KeyError(
                        'Non-optional property "{}" on {} is missing'.format(miss, self)
                    )
                )
        ...
        if len(set(jsondict.keys()) - valid) > 0:
            for supflu in set(jsondict.keys()) - valid:
                errs.append(
                    AttributeError(
                        'Superfluous entry "{}" in data for {}'.format(supflu, self)
                    )
                )
    ...
    def as_json(self):
        ...
        for (
            ...
            if is_list:
                if not isinstance(value, list):
                    err = TypeError(
                        'Expecting property "{}" on {} to be list, but is {}'.format(
                            name, type(self), type(value)
                        )
                    )
                elif len(value) > 0:
                    if not self._matches_type(value[0], typ):
                        err = TypeError(
                            'Expecting property "{}" on {} to be {}, but is {}'.format(
                                name, type(self), typ, type(value[0])
                            )
                        )
                    else:
                        lst = []
                ...
                if not self._matches_type(value, typ):
                    ...
                    try:
                        found.add(of_many or jsname)
                        js[jsname] = (
                            value.as_json() if hasattr(value, "as_json") else value
                        )
                    except FHIRValidationError as e:
                        err = e.prefixed(name)
        ...
        if len(nonoptionals - found) > 0:
            for nonop in nonoptionals - found:
                errs.append(
                    KeyError(
                        'Property "{}" on {} is not optional, you must provide a value for it'.format(
                            nonop, self
                        )
                    )
                )
