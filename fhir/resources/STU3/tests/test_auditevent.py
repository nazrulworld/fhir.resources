...
def impl_auditevent_1(inst):
    ...
    assert inst.agent[0].userId.value == "95"
    ...
    assert inst.agent[1].role[0].coding[0].code == "110153"
    assert inst.agent[1].role[0].coding[0].display == "Source Role ID"
    assert (
    ...
    assert inst.type.system == "http://hl7.org/fhir/audit-event-type"
...
def impl_auditevent_3(inst):
    ...
    assert (
        inst.entity[0].lifecycle.system == "http://hl7.org/fhir/dicom-audit-lifecycle"
    )
    assert inst.entity[0].reference.reference == "Patient/example/_history/1"
    assert inst.entity[0].type.code == "2"
    assert inst.entity[0].type.display == "System Object"
    assert inst.entity[0].type.system == "http://hl7.org/fhir/object-type"
...
def impl_auditevent_4(inst):
    ...
    assert inst.agent[0].userId.value == "ExportToMedia.app"
    ...
    assert (
        inst.entity[0].identifier.value
    ...
    assert inst.entity[0].type.system == "http://hl7.org/fhir/object-type"
    assert inst.entity[1].identifier.type.coding[0].code == "IHE XDS Metadata"
    assert (
        inst.entity[1].identifier.type.coding[0].display
        == "submission set classificationNode"
    )
    assert (
        inst.entity[1].identifier.type.coding[0].system
        == "urn:uuid:a54d6aa5-d40d-43f9-88c5-b4633d873bdd"
    )
    assert (
        inst.entity[1].identifier.value
        == "e3cdfc81a0d24bd^^^&2.16.840.1.113883.4.2&ISO"
    )
    ...
    assert inst.entity[2].reference.reference == "DocumentManifest/example"
    assert inst.entity[2].type.code == "2"
    assert inst.entity[2].type.display == "System Object"
    assert inst.entity[2].type.system == "http://hl7.org/fhir/object-type"
...
def impl_auditevent_6(inst):
    ...
    assert inst.agent[0].userId.value == "2.16.840.1.113883.4.2|2.16.840.1.113883.4.2"
    assert inst.agent[1].altId == "601847123"
    assert inst.agent[1].name == "Grahame Grieve"
    assert inst.agent[1].requestor is True
    assert inst.agent[1].userId.value == "95"
...
def impl_auditevent_7(inst):
    ...
    assert inst.agent[0].userId.value == "Grahame"
    assert inst.agent[1].altId == "6580"
    assert inst.agent[1].network.address == "Workstation1.ehr.familyclinic.com"
    assert inst.agent[1].network.type == "1"
    assert inst.agent[1].requestor is False
    ...
    assert inst.agent[1].userId.value == "2.16.840.1.113883.4.2|2.16.840.1.113883.4.2"
    assert inst.entity[0].identifier.type.coding[0].code == "SNO"
    assert (
        inst.entity[0].identifier.type.coding[0].system
        == "http://hl7.org/fhir/identifier-type"
    )
    assert inst.entity[0].identifier.type.text == "Dell Serial Number"
    assert inst.entity[0].identifier.value == "ABCDEF"
    assert inst.entity[0].lifecycle.code == "6"
    assert inst.entity[0].lifecycle.display == "Access / Use"
    assert (
        inst.entity[0].lifecycle.system == "http://hl7.org/fhir/dicom-audit-lifecycle"
    ...
    assert inst.source.identifier.value == "Grahame's Laptop"
...
def impl_auditevent_8(inst):
    ...
    assert inst.agent[0].userId.value == "SomeIdiot@nowhere.com"
    ...
    assert inst.agent[1].reference.reference == "Practitioner/example"
    assert inst.agent[1].requestor is False
    assert inst.agent[1].role[0].coding[0].code == "110152"
    assert inst.agent[1].role[0].coding[0].display == "Destination Role ID"
    assert (
        inst.agent[1].role[0].coding[0].system
        == "http://dicom.nema.org/resources/ontology/DCM"
    )
    assert inst.agent[1].userId.value == "Where"
    assert inst.entity[0].reference.reference == "Patient/example"
    assert inst.entity[0].role.code == "1"
    assert inst.entity[0].role.display == "Patient"
    assert inst.entity[0].role.system == "http://hl7.org/fhir/object-role"
    assert inst.entity[0].type.code == "1"
    ...
    assert inst.entity[1].identifier.value == "What.id"
    assert inst.entity[1].lifecycle.code == "11"
    assert inst.entity[1].lifecycle.display == "Disclosure"
    assert (
        inst.entity[1].lifecycle.system == "http://hl7.org/fhir/dicom-audit-lifecycle"
    )
    assert inst.entity[1].name == "Namne of What"
    assert inst.entity[1].reference.reference == "Patient/example/_history/1"
    ...
    assert inst.entity[1].securityLabel[1].system == "http://hl7.org/fhir/v3/ActCode"
    assert inst.entity[1].securityLabel[2].code == "DELAU"
    assert inst.entity[1].securityLabel[2].display == "delete after use"
    assert inst.entity[1].securityLabel[2].system == "http://hl7.org/fhir/v3/ActCode"
    assert inst.entity[1].type.code == "2"
    assert inst.entity[1].type.display == "System Object"
    assert inst.entity[1].type.system == "http://hl7.org/fhir/object-type"
    ...
    assert inst.purposeOfEvent[0].coding[0].system == "http://hl7.org/fhir/v3/ActReason"
    assert inst.recorded == fhirtypes.Instant.validate("2013-09-22T00:08:00Z")
    assert (
        inst.source.identifier.value == "Watchers Accounting of Disclosures Application"
    )
