...
def impl_careplan_4(inst):
    ...
    assert inst.activity[0].detail.status == "completed"
    assert (
        inst.activity[0].detail.statusReason
    ...
    assert inst.author[0].display == "Dr Adam Careful"
    assert inst.author[0].reference == "Practitioner/example"
    ...
    assert inst.context.reference == "Encounter/home"
    assert inst.definition[0].display == "A PlanDefinition protocol for obesity"
...
def impl_careplan_10(inst):
    ...
    assert inst.activity[3].detail.category.coding[0].code == "encounter"
    assert (
        inst.activity[3].detail.category.coding[0].system
        == "http://hl7.org/fhir/care-plan-activity-category"
    )
