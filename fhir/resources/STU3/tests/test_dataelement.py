...
class DataElementTests(unittest.TestCase):
    ...
    def implDataElement1(self, inst):
        ...
        self.assertEqual(
            force_bytes(inst.contained[1].id), force_bytes("2179414-permitted")
        )
        ...
        self.assertEqual(
            force_bytes(inst.element[0].binding.strength), force_bytes("required")
        )
        self.assertEqual(
            force_bytes(inst.element[0].code[0].code), force_bytes("46098-0")
        )
        self.assertEqual(
            force_bytes(inst.element[0].code[0].display), force_bytes("Sex")
        )
        self.assertEqual(
            force_bytes(inst.element[0].code[0].system), force_bytes("http://loinc.org")
        )
        self.assertEqual(
            force_bytes(inst.element[0].definition),
            force_bytes("The code representing the gender of a person."),
        )
        self.assertEqual(
            force_bytes(inst.element[0].extension[0].url),
            force_bytes("http://hl7.org/fhir/StructureDefinition/minLength"),
        )
        self.assertEqual(inst.element[0].extension[0].valueInteger, 1)
        self.assertEqual(
            force_bytes(inst.element[0].extension[1].url),
            force_bytes(
                "http://hl7.org/fhir/StructureDefinition/elementdefinition-question"
            ),
        )
        self.assertEqual(
            force_bytes(inst.element[0].extension[1].valueString), force_bytes("Gender")
        )
        self.assertEqual(
            force_bytes(inst.element[0].mapping[0].identity), force_bytes("fhir")
        )
        self.assertEqual(
            force_bytes(inst.element[0].mapping[0].language),
            force_bytes("application/xquery"),
        )
        self.assertEqual(
            force_bytes(inst.element[0].mapping[0].map),
            force_bytes("return f:/Patient/f:gender"),
        )
        self.assertEqual(inst.element[0].maxLength, 13)
        self.assertEqual(force_bytes(inst.element[0].path), force_bytes("Gender"))
        self.assertEqual(
            force_bytes(inst.element[0].type[0].code), force_bytes("CodeableConcept")
        )
        ...
        self.assertEqual(
            force_bytes(inst.mapping[0].name),
            force_bytes("Fast Healthcare Interoperable Resources (FHIR)"),
        )
        self.assertEqual(
            force_bytes(inst.mapping[0].uri), force_bytes("http://hl7.org/fhir/api")
        )
    ...
    def implDataElement2(self, inst):
        self.assertEqual(
            force_bytes(inst.element[0].alias[0]), force_bytes("Protime, PT")
        )
        self.assertEqual(
            force_bytes(inst.element[0].comment),
            force_bytes(
        ...
        self.assertEqual(
            force_bytes(inst.element[0].example[0].label), force_bytes("Simple")
        )
        ...
        self.assertEqual(
            force_bytes(
                inst.element[0].extension[0].valueCodeableConcept.coding[0].code
            ),
            force_bytes("s"),
        )
        self.assertEqual(
            force_bytes(
                inst.element[0].extension[0].valueCodeableConcept.coding[0].display
            ),
            force_bytes("second"),
        )
        self.assertEqual(
        ...
        self.assertTrue(
            inst.element[0].extension[0].valueCodeableConcept.coding[0].userSelected
        )
        self.assertEqual(
            force_bytes(
                inst.element[0].extension[0].valueCodeableConcept.coding[0].version
            ),
            force_bytes("1.9"),
        )
        self.assertEqual(
            force_bytes(inst.element[0].extension[0].valueCodeableConcept.text),
            force_bytes("second"),
        )
        self.assertEqual(
            force_bytes(inst.element[0].mapping[0].identity), force_bytes("loinc")
        )
        self.assertEqual(
            force_bytes(inst.element[0].mapping[0].map), force_bytes("5964-2")
        )
        self.assertEqual(force_bytes(inst.element[0].path), force_bytes("prothrombin"))
        self.assertEqual(
            force_bytes(inst.element[0].requirements),
            force_bytes(
                "This test is orderable. A plasma specimen in a 3.2% sodium citrate blue top tube is required."
            ),
        )
        self.assertEqual(
            force_bytes(inst.element[0].type[0].code), force_bytes("decimal")
        )
        self.assertEqual(force_bytes(inst.id), force_bytes("prothrombin"))
        self.assertEqual(
            inst.identifier[0].period.start.date, FHIRDate("2011-05-19").date
        )
        self.assertEqual(inst.identifier[0].period.start.as_json(), "2011-05-19")
        self.assertEqual(
            force_bytes(inst.identifier[0].system),
            force_bytes("http://www.CenturyHospital/Laboratory/DirectoryofServices"),
        )
        self.assertEqual(
            force_bytes(inst.identifier[0].type.text),
            force_bytes("Prothrombin Time, PT"),
        )
        self.assertEqual(force_bytes(inst.identifier[0].value), force_bytes("11"))
        self.assertEqual(
            force_bytes(inst.mapping[0].comment), force_bytes("Version 2.48 or later")
        )
        ...
        self.assertEqual(
            force_bytes(inst.mapping[0].uri), force_bytes("http://loinc.org/")
        )
