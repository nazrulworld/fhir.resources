...
class DocumentManifestTests(unittest.TestCase):
    ...
    def implDocumentManifest2(self, inst):
        ...
        self.assertEqual(force_bytes(inst.content[0].pAttachment.contentType), force_bytes("application/pdf"))
        self.assertEqual(inst.content[0].pAttachment.creation.date, FHIRDate("2010-02-01T11:50:23-05:00").date)
        self.assertEqual(inst.content[0].pAttachment.creation.as_json(), "2010-02-01T11:50:23-05:00")
        self.assertEqual(force_bytes(inst.content[0].pAttachment.data), force_bytes("SGVsbG8="))
        self.assertEqual(force_bytes(inst.content[0].pAttachment.title), force_bytes("accident notes 20100201.pdf"))
        self.assertEqual(force_bytes(inst.content[1].pAttachment.contentType), force_bytes("application/pdf"))
        self.assertEqual(inst.content[1].pAttachment.creation.date, FHIRDate("2010-02-01T10:57:34+01:00").date)
        self.assertEqual(inst.content[1].pAttachment.creation.as_json(), "2010-02-01T10:57:34+01:00")
        self.assertEqual(force_bytes(inst.content[1].pAttachment.hash), force_bytes("SGVsbG8gdGhlcmU="))
        self.assertEqual(inst.content[1].pAttachment.size, 104274)
        self.assertEqual(force_bytes(inst.content[1].pAttachment.url), force_bytes("http://happyvalley.com/docs/AB12345"))
        ...
        self.assertEqual(force_bytes(inst.identifier[0].system), force_bytes("http://happyvalley.com/supportingdocumentation"))
        self.assertEqual(force_bytes(inst.identifier[0].value), force_bytes("52345"))
        self.assertEqual(force_bytes(inst.related[0].identifier.system), force_bytes("http://happyvalley.com/claim"))
        self.assertEqual(force_bytes(inst.related[0].identifier.value), force_bytes("12345"))
        self.assertEqual(force_bytes(inst.related[1].identifier.system), force_bytes("http://www.BenefitsInc.com/fhir/remittance"))
        self.assertEqual(force_bytes(inst.related[1].identifier.value), force_bytes("R3500"))
        self.assertEqual(force_bytes(inst.status), force_bytes("current"))
        self.assertEqual(force_bytes(inst.text.div), force_bytes("<div xmlns=\"http://www.w3.org/1999/xhtml\">A Financial Management Attachment example</div>"))
        self.assertEqual(force_bytes(inst.text.status), force_bytes("generated"))
