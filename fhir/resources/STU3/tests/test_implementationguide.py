...
class ImplementationGuideTests(unittest.TestCase):
    ...
    def implImplementationGuide1(self, inst):
        self.assertEqual(
            force_bytes(inst.binary[0]), force_bytes("http://h7.org/fhir/fhir.css")
        )
        ...
        self.assertEqual(
            force_bytes(inst.dependency[0].uri),
        ...
        self.assertEqual(
            force_bytes(inst.package[0].description),
        ...
        self.assertEqual(
            force_bytes(inst.package[0].resource[0].acronym), force_bytes("daf-tst")
        )
        self.assertEqual(
            force_bytes(inst.package[0].resource[0].description),
            force_bytes("A test example to show how a package works"),
        )
        self.assertTrue(inst.package[0].resource[0].example)
        self.assertEqual(
            force_bytes(inst.package[0].resource[0].name), force_bytes("Test Example")
        )
        self.assertEqual(
            force_bytes(inst.package[0].resource[0].sourceUri), force_bytes("test.html")
        )
        self.assertEqual(force_bytes(inst.page.kind), force_bytes("page"))
        self.assertEqual(
            force_bytes(inst.page.page[0].format), force_bytes("text/html")
        )
        ...
        self.assertEqual(
            force_bytes(inst.page.page[0].source), force_bytes("list.html")
        )
        self.assertEqual(
            force_bytes(inst.page.page[0].title), force_bytes("Value Set Page")
        )
        self.assertEqual(
            force_bytes(inst.page.page[0].type[0]), force_bytes("ValueSet")
        )
        self.assertEqual(
            force_bytes(inst.page.source), force_bytes("patient-example.html")
        )
        self.assertEqual(
            force_bytes(inst.page.title), force_bytes("Example Patient Page")
        )
        self.assertEqual(
