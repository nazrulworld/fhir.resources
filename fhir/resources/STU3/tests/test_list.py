...
def impl_list_4(inst):
    ...
    assert inst.emptyReason.coding[0].system == "http://hl7.org/fhir/list-empty-reason"
...
def impl_list_7(inst):
    ...
    assert inst.entry[1].date == fhirtypes.DateTime.validate("2012-11-21")
...
def impl_list_8(inst):
    ...
    assert inst.orderedBy.coding[0].system == "http://hl7.org/fhir/list-order"
    assert inst.source.reference == "Patient/example"
    assert inst.status == "current"
    assert inst.text.status == "generated"
