...
def impl_medicationstatement_3(inst):
    ...
    assert inst.dosage[0].asNeededBoolean is False
    assert inst.dosage[0].doseQuantity.code == "tab"
    assert (
        inst.dosage[0].doseQuantity.system == "http://hl7.org/fhir/v3/orderableDrugForm"
    )
    assert inst.dosage[0].doseQuantity.unit == "tab"
...
def impl_medicationstatement_4(inst):
    ...
    assert (
        inst.note[0].text == "Patient can not take acetaminophen as per Dr instructions"
    )
    assert inst.reasonNotTaken[0].coding[0].code == "166643006"
    assert inst.reasonNotTaken[0].coding[0].display == "Liver enzymes abnormal"
    assert inst.reasonNotTaken[0].coding[0].system == "http://snomed.info/sct"
    assert inst.status == "active"
    assert inst.subject.display == "Donald Duck"
    assert inst.subject.reference == "Patient/pat1"
    assert inst.taken == "n"
...
def impl_medicationstatement_5(inst):
    ...
    assert (
        ...
        == "http://hl7.org/fhir/medication-statement-category"
    )
    assert inst.contained[0].id == "med0309"
    assert inst.dateAsserted == fhirtypes.DateTime.validate("2015-02-22")
    ...
    assert inst.dosage[0].doseRange.high.code == "TAB"
    assert (
        inst.dosage[0].doseRange.high.system
        == "http://hl7.org/fhir/v3/orderableDrugForm"
    )
    assert inst.dosage[0].doseRange.high.unit == "TAB"
    assert float(inst.dosage[0].doseRange.high.value) == float(2)
    assert inst.dosage[0].doseRange.low.code == "TAB"
    assert (
        inst.dosage[0].doseRange.low.system
        == "http://hl7.org/fhir/v3/orderableDrugForm"
    )
    assert inst.dosage[0].doseRange.low.unit == "TAB"
    assert float(inst.dosage[0].doseRange.low.value) == float(1)
...
def impl_medicationstatement_7(inst):
    assert inst.dateAsserted == fhirtypes.DateTime.validate("2014-02-22")
    assert inst.dosage[0].asNeededBoolean is False
    assert inst.dosage[0].doseQuantity.code == "mL"
    assert inst.dosage[0].doseQuantity.system == "http://unitsofmeasure.org"
    assert inst.dosage[0].doseQuantity.unit == "mL"
    assert float(inst.dosage[0].doseQuantity.value) == float(5)
    assert inst.dosage[0].maxDosePerPeriod.denominator.code == "d"
    assert (
        inst.dosage[0].maxDosePerPeriod.denominator.system
        == "http://unitsofmeasure.org"
