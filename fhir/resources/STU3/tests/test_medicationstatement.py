...
class MedicationStatementTests(unittest.TestCase):
    ...
    def implMedicationStatement3(self, inst):
        ...
        self.assertFalse(inst.dosage[0].asNeededBoolean)
        self.assertEqual(
            force_bytes(inst.dosage[0].doseQuantity.code), force_bytes("tab")
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseQuantity.system),
            force_bytes("http://hl7.org/fhir/v3/orderableDrugForm"),
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseQuantity.unit), force_bytes("tab")
        )
    ...
    def implMedicationStatement4(self, inst):
        ...
        self.assertEqual(
            force_bytes(inst.note[0].text),
            force_bytes("Patient can not take acetaminophen as per Dr instructions"),
        )
        self.assertEqual(
            force_bytes(inst.reasonNotTaken[0].coding[0].code), force_bytes("166643006")
        )
        self.assertEqual(
            force_bytes(inst.reasonNotTaken[0].coding[0].display),
            force_bytes("Liver enzymes abnormal"),
        )
        self.assertEqual(
            force_bytes(inst.reasonNotTaken[0].coding[0].system),
            force_bytes("http://snomed.info/sct"),
        )
        self.assertEqual(force_bytes(inst.status), force_bytes("active"))
    ...
    def implMedicationStatement5(self, inst):
        ...
        self.assertEqual(
            ...
            force_bytes("http://hl7.org/fhir/medication-statement-category"),
        )
        self.assertEqual(force_bytes(inst.contained[0].id), force_bytes("med0309"))
        self.assertEqual(inst.dateAsserted.date, FHIRDate("2015-02-22").date)
        self.assertEqual(inst.dateAsserted.as_json(), "2015-02-22")
        ...
        self.assertEqual(
            force_bytes(inst.dosage[0].doseRange.high.code), force_bytes("TAB")
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseRange.high.system),
            force_bytes("http://hl7.org/fhir/v3/orderableDrugForm"),
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseRange.high.unit), force_bytes("TAB")
        )
        self.assertEqual(inst.dosage[0].doseRange.high.value, 2)
        self.assertEqual(
            force_bytes(inst.dosage[0].doseRange.low.code), force_bytes("TAB")
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseRange.low.system),
            force_bytes("http://hl7.org/fhir/v3/orderableDrugForm"),
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseRange.low.unit), force_bytes("TAB")
        )
    ...
    def implMedicationStatement7(self, inst):
        ...
        self.assertFalse(inst.dosage[0].asNeededBoolean)
        self.assertEqual(
            force_bytes(inst.dosage[0].doseQuantity.code), force_bytes("mL")
        )
        self.assertEqual(
            force_bytes(inst.dosage[0].doseQuantity.system),
            force_bytes("http://unitsofmeasure.org"),
        ...
        self.assertEqual(
            force_bytes(inst.dosage[0].doseQuantity.unit), force_bytes("mL")
        )
