...
class NamingSystemTests(unittest.TestCase):
    ...
    def implNamingSystem1(self, inst):
        self.assertEqual(
            force_bytes(inst.contact[0].name), force_bytes("HL7 Australia FHIR Team")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            force_bytes("http://hl7-australia.wikispaces.com/FHIR+Australia"),
        )
        ...
        self.assertEqual(
            force_bytes(inst.jurisdiction[0].coding[0].code), force_bytes("AU")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            force_bytes("Austalian Healthcare Identifier - Individual"),
        )
        self.assertEqual(
            force_bytes(inst.publisher), force_bytes("HL7 Australia on behalf of NEHTA")
        )
        self.assertEqual(
            force_bytes(inst.responsible), force_bytes("HI Service Operator / NEHTA")
        )
        ...
        self.assertEqual(
            ...
            force_bytes("National unique individual identifier"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            force_bytes(inst.uniqueId[0].comment),
            force_bytes("This value is used in Australian CDA documents"),
        )
        self.assertEqual(force_bytes(inst.uniqueId[0].type), force_bytes("oid"))
        self.assertEqual(
            force_bytes(inst.uniqueId[0].value), force_bytes("1.2.36.1.2001.1003.0")
        )
        self.assertEqual(
            inst.uniqueId[1].period.start.date, FHIRDate("2015-08-21").date
        )
        ...
        self.assertEqual(
            force_bytes(inst.uniqueId[1].value),
            force_bytes("http://ns.electronichealth.net.au/id/hi/ihi/1.0"),
        )
        self.assertEqual(
            force_bytes(inst.usage),
            force_bytes("Used in Australia for identifying patients"),
        )
    ...
    def implNamingSystem2(self, inst):
        ...
        self.assertEqual(force_bytes(inst.uniqueId[0].type), force_bytes("oid"))
        self.assertEqual(
            force_bytes(inst.uniqueId[0].value), force_bytes("2.16.840.1.113883.6.96.1")
        )
    ...
    def implNamingSystem3(self, inst):
        self.assertEqual(
            force_bytes(inst.contact[0].name), force_bytes("FHIR project team")
        )
        self.assertEqual(
            force_bytes(inst.contact[0].telecom[0].system), force_bytes("url")
        )
        ...
        self.assertEqual(
            ...
            force_bytes("HL7 International on behalf of IHTSDO"),
        )
        self.assertEqual(
            force_bytes(inst.responsible), force_bytes("IHTSDO & affiliates")
        )
        self.assertEqual(force_bytes(inst.status), force_bytes("active"))
        self.assertEqual(force_bytes(inst.text.status), force_bytes("generated"))
        self.assertEqual(force_bytes(inst.uniqueId[0].type), force_bytes("oid"))
        self.assertEqual(
            force_bytes(inst.uniqueId[0].value), force_bytes("2.16.840.1.113883.6.96")
        )
        self.assertTrue(inst.uniqueId[1].preferred)
        self.assertEqual(force_bytes(inst.uniqueId[1].type), force_bytes("uri"))
        self.assertEqual(
            force_bytes(inst.uniqueId[1].value), force_bytes("http://snomed.info/sct")
        )
