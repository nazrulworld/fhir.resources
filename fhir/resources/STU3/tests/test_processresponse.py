...
class ProcessResponseTests(unittest.TestCase):
    ...
    def implProcessResponse1(self, inst):
        ...
        self.assertEqual(
            ...
            force_bytes("http://www.BenefitsInc.com/fhir/processresponse"),
        )
    ...
    def implProcessResponse2(self, inst):
        ...
        self.assertEqual(
            force_bytes(inst.disposition),
            force_bytes("Referred to claim not found on system."),
        )
        self.assertEqual(force_bytes(inst.error[0].coding[0].code), force_bytes("a001"))
        self.assertEqual(
            force_bytes(inst.error[0].coding[0].system),
            force_bytes("http://hl7.org/fhir/adjudication-error"),
        )
        self.assertEqual(
            force_bytes(inst.form.coding[0].code), force_bytes("PRRESP/2016/01")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            force_bytes(inst.processNote[0].type.coding[0].code), force_bytes("print")
        )
        self.assertEqual(
    ...
    def implProcessResponse3(self, inst):
        ...
        self.assertEqual(
            force_bytes(inst.disposition),
            force_bytes("Additional information required."),
        )
        ...
        self.assertEqual(
            force_bytes(inst.outcome.coding[0].code), force_bytes("pended")
        )
        self.assertEqual(
            force_bytes(inst.outcome.coding[0].system),
            force_bytes("http://hl7.org/fhir/processoutcomecodes"),
