...
class ServiceDefinitionTests(unittest.TestCase):
    ...
    def implServiceDefinition1(self, inst):
        self.assertEqual(force_bytes(inst.dataRequirement[0].mustSupport[0]), force_bytes("gender"))
        self.assertEqual(force_bytes(inst.dataRequirement[0].mustSupport[1]), force_bytes("birthDate"))
        ...
        self.assertEqual(force_bytes(inst.dataRequirement[2].type), force_bytes("MedicationStatement"))
        self.assertEqual(inst.date.date, FHIRDate("2016-07-05").date)
        self.assertEqual(inst.date.as_json(), "2016-07-05")
        self.assertEqual(force_bytes(inst.description), force_bytes("The InfoButton specification defines a mechanism for retrieving relevant clinical context based a particular set of search criteria.."))
        ...
        self.assertEqual(force_bytes(inst.identifier[0].value), force_bytes("infobutton"))
        self.assertEqual(force_bytes(inst.status), force_bytes("draft"))
        self.assertEqual(force_bytes(inst.text.status), force_bytes("generated"))
        self.assertEqual(force_bytes(inst.title), force_bytes("InfoButton Module"))
        self.assertEqual(force_bytes(inst.topic[0].text), force_bytes("InfoButton Module"))
