...
def impl_specimen_2(inst):
    ...
    assert (
        inst.processing[0].procedure.coding[0].system == "http://hl7.org/fhir/v2/0373"
    )
...
def impl_specimen_3(inst):
    ...
    assert inst.id == "sst"
    assert inst.request[0].reference == "ProcedureRequest/ft4"
...
def impl_specimen_4(inst):
    ...
    assert inst.request[0].reference == "ProcedureRequest/example"
    assert inst.status == "available"
