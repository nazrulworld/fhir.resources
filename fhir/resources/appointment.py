...
class Appointment(domainresource.DomainResource):
    ...
    "the process, however this status indicates the shared status."
		...
		enum_values=["proposed", "pending", "booked", "arrived", "fulfilled", "cancelled", "noshow", "entered-in-error", "checked-in", "waitlist"],
	)
    ...
    def elements_sequence(cls):
        ...
        """
        return ["id", "meta", "implicitRules", "language", "text", "contained", "extension", "modifierExtension", "identifier", "status", "cancelationReason", "serviceCategory", "serviceType", "specialty", "appointmentType", "reasonCode", "reasonReference", "priority", "description", "supportingInformation", "start", "end", "minutesDuration", "slot", "created", "comment", "patientInstruction", "basedOn", "participant", "requestedPeriod"]
...
class AppointmentParticipant(backboneelement.BackboneElement):
    ...
    def elements_sequence(cls):
        ...
        with preserving original sequence order.
        """
        return ["id", "extension", "modifierExtension", "type", "actor", "required", "status", "period"]
