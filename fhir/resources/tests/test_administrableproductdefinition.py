...
def impl_administrableproductdefinition_1(inst):
    ...
    assert inst.administrableDoseForm.coding[0].system == "http://ema.europa.eu/example/administrabledoseform"
    assert inst.id == "example"
    assert inst.identifier[0].system == "http://ema.europa.eu/example/phpididentifiersets"
...
def test_administrableproductdefinition_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "administrableproductdefinition-example.json"
    )
