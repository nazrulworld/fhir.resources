...
def impl_basic_1(inst):
    ...
    assert inst.code.coding[0].system == "http://example.org/do-not-use/fhir-codes#resourceTypes"
    ...
    assert inst.extension[0].url == "http://example.org/do-not-use/fhir-extensions/UMLclass"
...
def test_basic_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "basic-example2.json"
    )
...
def impl_basic_3(inst):
    ...
    assert inst.code.coding[0].system == "http://terminology.hl7.org/CodeSystem/basic-resource-type"
    ...
    assert inst.extension[1].url == "http://example.org/do-not-use/fhir-extensions/referral#notes"
    ...
    "http://example.org/do-not-use/fhir-"
    "extensions/referral#fulfillingEncounter"
    )
    ...
    "http://example.org/do-not-use/fhir-"
    "extensions/referral#referredForService"
    )
    assert inst.modifierExtension[0].valueCodeableConcept.coding[0].code == "11429006"
    assert inst.modifierExtension[0].valueCodeableConcept.coding[0].display == "Consultation"
    assert inst.modifierExtension[0].valueCodeableConcept.coding[0].system == "http://snomed.info/sct"
    assert inst.modifierExtension[1].url == (
    "http://example.org/do-not-use/fhir-"
    "extensions/referral#targetDate"
    )
    assert inst.modifierExtension[1].valuePeriod.end == fhirtypes.DateTime.validate("2013-04-15")
    assert inst.modifierExtension[1].valuePeriod.start == fhirtypes.DateTime.validate("2013-04-01")
    assert inst.modifierExtension[2].url == (
    "http://example.org/do-not-use/fhir-"
    "extensions/referral#status"
    )
...
def test_basic_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "basic-example.json"
    )
    inst = basic.Basic.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
