...
def impl_biologicallyderivedproduct_1(inst):
    assert (
        ...
        == "https://www.isbt128.org/uri/DonationIdentificationNumber"
    )
    assert inst.biologicalSourceEvent.value == "A999921123456"
    assert inst.division == "A00000"
    assert inst.expirationDate == fhirtypes.DateTime.validate(
        "2021-08-02T23:59:00-05:00"
    )
    assert inst.id == "example"
    assert (
        inst.identifier[0].system == "https://www.isbt128.org/uri/MPHOUniqueIdentifier"
    )
    ...
    assert (
        ...
        == "https://www.isbt128.org/uri/ABORhD"
    )
    ...
    assert (
        ...
        == "https://www.isbt128.org/uri/CollectionType"
    )
