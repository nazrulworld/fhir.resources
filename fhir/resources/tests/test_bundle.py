...
def impl_bundle_1(inst):
    ...
    assert (
        ...
        == ExternalValidatorModel(valueUri="Encounter/2").valueUri
    )
    assert inst.entry[1].resource.id == "2"
    assert (
        inst.entry[1].resource.meta.lastUpdated
        == ExternalValidatorModel(valueInstant="2019-08-07T10:49:22Z").valueInstant
    )
...
def impl_bundle_2(inst):
    ...
    assert (
        ...
        == ExternalValidatorModel(valueInstant="2017-03-14T08:23:30+11:00").valueInstant
    )
    assert inst.meta.tag[0].code == "HTEST"
    assert inst.meta.tag[0].display == "test health data"
    assert (
...
def impl_bundle_5(inst):
    ...
    assert (
        inst.timestamp
        == ExternalValidatorModel(valueInstant="2015-07-14T11:15:33+10:00").valueInstant
    )
...
def impl_bundle_6(inst):
    ...
    assert (
        ...
        == ExternalValidatorModel(valueUri="DocumentReference").valueUri
    )
    assert (
        inst.entry[0].resource.meta.lastUpdated
        == ExternalValidatorModel(valueInstant="2013-07-01T13:11:33Z").valueInstant
    )
    assert (
        inst.entry[1].fullUrl
        == ExternalValidatorModel(
    ...
    assert (
        inst.entry[1].request.url == ExternalValidatorModel(valueUri="Patient").valueUri
    )
    ...
    assert (
        inst.entry[4].request.url == ExternalValidatorModel(valueUri="Binary").valueUri
    )
