...
def impl_bundle_1(inst):
    ...
    assert inst.entry[1].resource.meta.lastUpdated == fhirtypes.Instant.validate("2019-08-07T10:49:22Z")
...
def test_bundle_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "notification-full-resource.json"
    )
...
def impl_bundle_2(inst):
    ...
    assert inst.meta.lastUpdated == fhirtypes.Instant.validate("2017-03-14T08:23:30+11:00")
    assert inst.meta.tag[0].code == "HTEST"
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.total == 0
...
def test_bundle_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "bundle-search-warning.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_bundle_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "notification-id-only-query.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_bundle_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "notification-heartbeat.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_bundle_6(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "message-response-link.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_bundle_7(inst):
    ...
    assert inst.entry[0].resource.meta.lastUpdated == fhirtypes.Instant.validate("2013-07-01T13:11:33Z")
    assert inst.entry[1].fullUrl == "http://localhost:9556/svc/fhir/Patient/a2"
    assert inst.entry[1].request.ifNoneExist == "Patient?identifier=http://acme.org/xds/patients!89765a87b"
    assert inst.entry[1].request.method == "POST"
    assert inst.entry[1].request.url == "Patient"
    assert inst.entry[1].resource.id == "a2"
    assert inst.entry[1].resource.meta.lastUpdated == fhirtypes.Instant.validate("2013-07-01T13:11:33Z")
    ...
    assert inst.entry[2].resource.meta.lastUpdated == fhirtypes.Instant.validate("2013-07-01T13:11:33Z")
    ...
    assert inst.entry[3].resource.meta.lastUpdated == fhirtypes.Instant.validate("2013-07-01T13:11:33Z")
    assert inst.entry[4].fullUrl == (
    "http://localhost:9556/svc/fhir/Binary/1e404af3-077f-4bee-b7a"
    "6-a9be97e1ce32"
    )
    ...
    assert inst.entry[4].resource.meta.lastUpdated == fhirtypes.Instant.validate("2013-07-01T13:11:33Z")
    assert inst.id == "xds"
...
def test_bundle_7(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "xds-example.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_bundle_8(inst):
    assert inst.entry[0].fullUrl == "http://hl7.org/fhir/SearchParameter/DomainResource-text"
    assert inst.entry[0].resource.id == "DomainResource-text"
    assert inst.entry[1].fullUrl == "http://hl7.org/fhir/SearchParameter/Resource-content"
    assert inst.entry[1].resource.id == "Resource-content"
    assert inst.entry[2].fullUrl == "http://hl7.org/fhir/SearchParameter/Resource-filter"
    ...
    assert inst.entry[5].fullUrl == "http://hl7.org/fhir/SearchParameter/Resource-lastUpdated"
    ...
    assert inst.entry[7].fullUrl == "http://hl7.org/fhir/SearchParameter/Resource-profile"
    ...
    assert inst.entry[9].fullUrl == "http://hl7.org/fhir/SearchParameter/Resource-security"
...
def test_bundle_8(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "search-parameters.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_bundle_9(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "message-request-link.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_bundle_10(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "namingsystem-terminologies.json"
    )
    inst = bundle.Bundle.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
