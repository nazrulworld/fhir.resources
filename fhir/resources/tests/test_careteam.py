...
class CareTeamTests(unittest.TestCase):
    ...
    def implCareTeam1(self, inst):
        self.assertEqual(
            force_bytes(inst.category[0].coding[0].code), force_bytes("LA27976-2")
        )
        self.assertEqual(
            force_bytes(inst.category[0].coding[0].display),
            force_bytes("Encounter-focused care team"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            force_bytes(inst.participant[0].role[0].text),
            force_bytes("responsiblePerson"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            force_bytes(inst.participant[1].role[0].text), force_bytes("adviser")
        )
