...
def impl_chargeitem_1(inst):
    ...
    "Zusatzpauschale für Beobachtung nach diagnostischer "
    "Koronarangiografie"
    )
    assert inst.context.reference == "Encounter/example"
    assert inst.definitionUri[0] == (
    "http://www.kbv.de/tools/ebm/html/01520_290436086082622081363"
    "2.html"
    )
    ...
    assert inst.occurrencePeriod.end == fhirtypes.DateTime.validate("2017-01-25T12:35:00+01:00")
    assert inst.occurrencePeriod.start == fhirtypes.DateTime.validate("2017-01-25T08:00:00+01:00")
    assert inst.overrideReason == (
    "Patient is Cardiologist's golf buddy, so he gets a 20% "
    "discount!"
    )
    ...
    assert inst.performingOrganization.identifier.system == "http://myhospital/NamingSystem/departments"
    ...
    assert inst.requestingOrganization.identifier.system == "http://myhospital/NamingSystem/departments"
...
def test_chargeitem_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "chargeitem-example.json"
    )
