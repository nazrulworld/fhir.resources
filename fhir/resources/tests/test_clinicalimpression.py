...
def impl_clinicalimpression_1(inst):
    ...
    assert inst.effectivePeriod.end == fhirtypes.DateTime.validate("2014-12-06T22:33:00+11:00")
    assert inst.effectivePeriod.start == fhirtypes.DateTime.validate("2014-12-06T20:00:00+11:00")
    assert inst.encounter.reference == "Encounter/example"
    assert inst.finding[0].itemCodeableConcept.coding[0].code == "850.0"
    assert inst.finding[0].itemCodeableConcept.coding[0].system == "http://hl7.org/fhir/sid/icd-9"
    assert inst.id == "example"
    assert inst.identifier[0].value == "12345"
    assert inst.investigation[0].code.text == "Initial Examination"
    assert inst.investigation[0].item[0].display == "deep laceration of the scalp (left temporo-occipital)"
...
def test_clinicalimpression_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "clinicalimpression-example.json"
    )
