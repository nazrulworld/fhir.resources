...
def test_codesystem_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-map-model-mode.json"
    )
...
def impl_codesystem_2(inst):
    ...
    "http://hl7.org/fhir/StructureDefinition/codesystem-concept-"
    "comments"
    )
    ...
    assert inst.concept[5].definition == "The are no known applicable values in this context."
    assert inst.concept[5].display == "Nil Known"
    assert inst.concept[5].extension[0].url == (
    "http://hl7.org/fhir/StructureDefinition/codesystem-concept-"
    "comments"
    )
    assert inst.concept[5].extension[0].valueString == "The existence of this subject to review"
    assert inst.content == "complete"
...
def test_codesystem_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-special-values.json"
    )
    inst = codesystem.CodeSystem.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_codesystem_3(inst):
    ...
    assert inst.concept[0].definition == "The communication was not done due to an unknown reason."
    assert inst.concept[0].display == "Unknown"
    assert inst.concept[1].code == "system-error"
    assert inst.concept[1].definition == "The communication was not done due to a system error."
    assert inst.concept[1].display == "System Error"
    assert inst.concept[2].code == "invalid-phone-number"
    assert inst.concept[2].definition == (
    "The communication was not done due to an invalid phone "
    "number."
    )
    assert inst.concept[2].display == "Invalid Phone Number"
    assert inst.concept[3].code == "recipient-unavailable"
    assert inst.concept[3].definition == (
    "The communication was not done due to the recipient being "
    "unavailable."
    )
    assert inst.concept[3].display == "Recipient Unavailable"
    assert inst.concept[4].code == "family-objection"
    assert inst.concept[4].definition == "The communication was not done due to a family objection."
    assert inst.concept[4].display == "Family Objection"
    assert inst.concept[5].code == "patient-objection"
    assert inst.concept[5].definition == "The communication was not done due to a patient objection."
    ...
    assert inst.content == "complete"
    assert inst.date == fhirtypes.DateTime.validate("2021-01-17T07:06:13+11:00")
    assert inst.description == "Codes for the reason why a communication did not happen."
    assert inst.experimental is False
    assert inst.extension[0].url == (
    "http://hl7.org/fhir/StructureDefinition/structuredefinition-"
    "wg"
    ...
    assert inst.url == (
    "http://terminology.hl7.org/CodeSystem/communication-not-"
    "done-reason"
    )
...
def test_codesystem_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-communication-not-done-reason.json"
    )
    inst = codesystem.CodeSystem.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_codesystem_4(inst):
    ...
    assert inst.content == "complete"
    assert inst.date == fhirtypes.DateTime.validate("2021-01-17T07:06:13+11:00")
    assert inst.description == "The meaning of the hierarchy of concepts in a code system."
    assert inst.experimental is False
    assert inst.extension[0].url == (
    "http://hl7.org/fhir/StructureDefinition/structuredefinition-"
    "wg"
...
def test_codesystem_4(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-codesystem-hierarchy-meaning.json"
    )
    inst = codesystem.CodeSystem.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_codesystem_5(inst):
    ...
    "http://hl7.org/fhir/ValueSet/medicationrequest-course-of-"
    "therapy"
    )
    assert inst.version == "4.3.0"
...
def test_codesystem_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-medicationrequest-course-of-therapy.json"
    )
    inst = codesystem.CodeSystem.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_codesystem_6(inst):
    ...
    assert inst.concept[0].definition == (
    "The operation completed successfully (whether with warnings "
    "or not)."
    )
...
def impl_codesystem_7(inst):
    ...
    assert inst.concept[0].definition == "Canadian health information disclosure policy."
    assert inst.concept[0].display == "Disclosure-CA"
    assert inst.concept[1].code == "disclosure-us"
    assert inst.concept[1].definition == "United States health information disclosure policy."
...
def test_codesystem_7(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-contract-subtype.json"
    )
    inst = codesystem.CodeSystem.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_codesystem_8(inst):
    ...
    assert inst.concept[2].definition == "truncate(source, length) - source must be stringy type."
    ...
    assert inst.concept[5].definition == "append(source...) - source is element or string."
    assert inst.concept[5].display == "append"
    assert inst.concept[6].code == "translate"
    assert inst.concept[6].definition == "translate(source, uri_of_map) - use the translate operation."
    ...
    assert inst.concept[8].definition == "Perform a date operation. *Parameters to be documented*."
    assert inst.concept[8].display == "dateOp"
    assert inst.concept[9].code == "uuid"
    assert inst.concept[9].definition == "Generate a random UUID (in lowercase). No Parameters."
...
def test_codesystem_8(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "codesystem-map-transform.json"
    )
    inst = codesystem.CodeSystem.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_codesystem_9(inst):
    ...
    assert inst.concept[0].definition == "Only contraindicated if the other therapy is given"
    assert inst.concept[0].display == "Only contraindicated if the other therapy is given"
    assert inst.concept[1].code == "contraindicated-except-with"
    assert inst.concept[1].definition == "Contraindicated unless the other therapy is given"
    assert inst.concept[1].display == "Contraindicated unless the other therapy is given"
    assert inst.concept[2].code == "indicated-only-with"
    assert inst.concept[2].definition == (
    "Indicated only when the other therapy is given (co-"
    "occurrent)"
    )
    ...
    assert inst.concept[3].definition == "Indicated except when the other therapy is given"
    ...
    "Indicated only if the other therapy was given before "
    "(follow-up)"
    )
    ...
    assert inst.concept[7].definition == "Indicated to replace the other contraindicated therapy."
    assert inst.concept[7].display == "Indicated to replace the other contraindicated therapy"
    assert inst.concept[8].code == "replace-other-therapy-not-tolerated"
    assert inst.concept[8].definition == (
    "Indicated to replace the other therapy not well tolerated by"
    " patient"
    )
    ...
    "Indicated to replace the other therapy not effective on "
    "patient"
    )
...
def impl_codesystem_10(inst):
    ...
    assert inst.concept[1].definition == (
    "At least one instance has been associated with this imaging "
    "study."
    )
