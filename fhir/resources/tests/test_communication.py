...
def impl_communication_1(inst):
    ...
    assert inst.about[1].identifier.system == "http://www.BenefitsInc.com/fhir/claimresponse"
    ...
    assert inst.payload[0].contentAttachment.creation == fhirtypes.DateTime.validate("2010-02-01T11:50:23-05:00")
    ...
    assert inst.payload[1].contentAttachment.creation == fhirtypes.DateTime.validate("2010-02-01T10:57:34+01:00")
    ...
    assert inst.sender.identifier.system == "http://www.jurisdiction.com/provideroffices"
...
def impl_communication_2(inst):
    ...
    assert inst.payload[1].contentAttachment.hash == bytes_validator("SGVsbG8gdGhlcmU=")
    assert inst.payload[1].contentAttachment.size == 104274
    assert inst.payload[1].contentAttachment.url == "http://happyvalley.com/docs/AB12345"
...
def test_communication_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "communication-example-fm-solicited-attachment.json"
    )
    inst = communication.Communication.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_communication_3(inst):
    ...
    assert inst.medium[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/v3-ParticipationMode"
...
def test_communication_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "communication-example.json"
    )
    inst = communication.Communication.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
