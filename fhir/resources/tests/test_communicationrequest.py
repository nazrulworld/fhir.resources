...
def test_communicationrequest_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "communicationrequest-example.json"
    )
...
def impl_communicationrequest_2(inst):
    ...
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.occurrenceDateTime == fhirtypes.DateTime.validate("2016-06-10T11:01:10-08:00")
    ...
    assert inst.status == "active"
    assert inst.text.div == (
    "<div xmlns=\"http://www.w3.org/1999/xhtml\">Request for "
    "Accident Report</div>"
    )
    assert inst.text.status == "generated"
...
def test_communicationrequest_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "communicationrequest-example-fm-solicit-attachment.json"
    )
    inst = communicationrequest.CommunicationRequest.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
