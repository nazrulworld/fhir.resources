...
def impl_coverageeligibilityrequest_1(inst):
    ...
    assert inst.identifier[0].system == "http://happyvalley.com/coverageelegibilityrequest"
...
def impl_coverageeligibilityrequest_2(inst):
    ...
    assert inst.item[0].category.coding[0].system == "http://terminology.hl7.org/CodeSystem/ex-benefitcategory"
    assert inst.meta.tag[0].code == "HTEST"
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.patient.reference == "Patient/pat1"
