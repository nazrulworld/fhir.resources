...
class DeviceMetricTests(unittest.TestCase):
    ...
    def implDeviceMetric1(self, inst):
        self.assertEqual(
            force_bytes(inst.calibration[0].state), force_bytes("calibrated")
        )
        self.assertEqual(
            inst.calibration[0].time.date, FHIRDate("2016-12-28T09:03:04-05:00").date
        )
        self.assertEqual(
            inst.calibration[0].time.as_json(), "2016-12-28T09:03:04-05:00"
        )
        self.assertEqual(
            force_bytes(inst.calibration[0].type), force_bytes("two-point")
        )
        ...
        self.assertEqual(
            ...
            force_bytes("http://goodcare.org/devicemetric/id"),
        )
        ...
        self.assertEqual(
            force_bytes(inst.measurementPeriod.repeat.periodUnit), force_bytes("s")
        )
        ...
        self.assertEqual(
            ...
            force_bytes("MDC_PULS_OXIM_SAT_O2"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            force_bytes(inst.unit.coding[0].display), force_bytes("MDC_DIM_PERCENT")
        )
        self.assertEqual(
            force_bytes(inst.unit.coding[0].system),
            force_bytes("urn:iso:std:iso:11073:10101"),
        )
