...
def impl_diagnosticreport_1(inst):
    ...
    assert inst.media[0].link.reference == "Media/1.2.840.11361907579238403408700.3.1.04.19970327150033"
...
def impl_diagnosticreport_5(inst):
    ...
    assert inst.effectiveDateTime == fhirtypes.DateTime.validate("2016-10-15T12:34:56+11:00")
    ...
    assert inst.presentedForm[0].contentType == "application/pdf"
    assert inst.presentedForm[0].creation == fhirtypes.DateTime.validate("2016-10-20T20:00:00+11:00")
    assert inst.presentedForm[0].data == bytes_validator("cGRmSW5CYXNlNjRCaW5hcnk=")
    assert inst.presentedForm[0].hash == bytes_validator("571ef9c5655840f324e679072ed62b1b95eef8a0")
    assert inst.presentedForm[0].language == "en"
...
def test_diagnosticreport_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "diagnosticreport-example-pgx.json"
    )
    inst = diagnosticreport.DiagnosticReport.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
