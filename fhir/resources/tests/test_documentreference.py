...
def impl_documentreference_1(inst):
    ...
    assert inst.category[0].coding[0].system == "http://ihe.net/xds/connectathon/classCodes"
    assert inst.contained[0].id == "a2"
    assert inst.content[0].attachment.contentType == "application/hl7-v3+xml"
    assert inst.content[0].attachment.creation == fhirtypes.DateTime.validate("2005-12-24T09:35:00+11:00")
    assert inst.content[0].attachment.hash == bytes_validator("2jmj7l5rSw0yVb/vlWAYkK/YBwk=")
    ...
    assert inst.context.event[0].coding[0].system == "http://ihe.net/xds/connectathon/eventCodes"
    ...
    assert inst.context.period.end == fhirtypes.DateTime.validate("2004-12-23T08:01:00+11:00")
    assert inst.context.period.start == fhirtypes.DateTime.validate("2004-12-23T08:00:00+11:00")
...
def test_documentreference_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "documentreference-example.json"
    )
