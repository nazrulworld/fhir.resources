...
def impl_encounter_3(inst):
    ...
    assert inst.hospitalization.dischargeDisposition.coding[0].display == "Discharge to home"
    assert inst.hospitalization.dischargeDisposition.coding[0].system == "http://snomed.info/sct"
    assert inst.hospitalization.preAdmissionIdentifier.system == "http://www.bmc.nl/zorgportal/identifiers/pre-admissions"
...
def impl_encounter_5(inst):
    ...
    assert inst.hospitalization.admitSource.coding[0].display == "Referral by radiologist"
    assert inst.hospitalization.admitSource.coding[0].system == "http://snomed.info/sct"
    assert inst.hospitalization.dischargeDisposition.coding[0].code == "306689006"
    assert inst.hospitalization.dischargeDisposition.coding[0].display == "Discharge to home"
    assert inst.hospitalization.dischargeDisposition.coding[0].system == "http://snomed.info/sct"
...
def test_encounter_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "encounter-example-f002-lung.json"
    )
    inst = encounter.Encounter.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_encounter_6(inst):
    ...
    assert inst.hospitalization.admitSource.coding[0].display == "Clinical Oncology Department"
    assert inst.hospitalization.admitSource.coding[0].system == "http://snomed.info/sct"
    ...
    assert inst.hospitalization.dietPreference[0].coding[0].display == "Fluid balance regulation"
    assert inst.hospitalization.dietPreference[0].coding[0].system == "http://snomed.info/sct"
    ...
    assert inst.hospitalization.specialCourtesy[0].coding[0].display == "normal courtesy"
...
def impl_encounter_7(inst):
    ...
    assert inst.reasonCode[0].coding[0].system == "http://ihe.net/xds/connectathon/eventCodes"
    assert inst.status == "finished"
...
def impl_encounter_9(inst):
    ...
    assert inst.hospitalization.admitSource.coding[0].display == "From accident/emergency department"
    assert inst.hospitalization.admitSource.coding[0].system == "http://terminology.hl7.org/CodeSystem/admit-source"
    ...
    assert inst.status == "in-progress"
    assert inst.statusHistory[0].period.end == fhirtypes.DateTime.validate("2017-02-01T07:35:00+10:00")
    assert inst.statusHistory[0].period.start == fhirtypes.DateTime.validate("2017-02-01T07:15:00+10:00")
    assert inst.statusHistory[0].status == "arrived"
    assert inst.statusHistory[1].period.end == fhirtypes.DateTime.validate("2017-02-01T08:45:00+10:00")
    assert inst.statusHistory[1].period.start == fhirtypes.DateTime.validate("2017-02-01T07:35:00+10:00")
    assert inst.statusHistory[1].status == "triaged"
    assert inst.statusHistory[2].period.end == fhirtypes.DateTime.validate("2017-02-01T12:15:00+10:00")
    assert inst.statusHistory[2].period.start == fhirtypes.DateTime.validate("2017-02-01T08:45:00+10:00")
    assert inst.statusHistory[2].status == "in-progress"
    assert inst.statusHistory[3].period.end == fhirtypes.DateTime.validate("2017-02-01T12:45:00+10:00")
    assert inst.statusHistory[3].period.start == fhirtypes.DateTime.validate("2017-02-01T12:15:00+10:00")
    assert inst.statusHistory[3].status == "onleave"
    assert inst.statusHistory[4].period.start == fhirtypes.DateTime.validate("2017-02-01T12:45:00+10:00")
...
def impl_encounter_10(inst):
    ...
    assert inst.hospitalization.dischargeDisposition.coding[0].display == "Discharge to home"
    assert inst.hospitalization.dischargeDisposition.coding[0].system == "http://snomed.info/sct"
    assert inst.hospitalization.preAdmissionIdentifier.system == "http://www.amc.nl/zorgportal/identifiers/pre-admissions"
    assert inst.hospitalization.preAdmissionIdentifier.use == "official"
    assert inst.hospitalization.preAdmissionIdentifier.value == "93042"
    assert inst.id == "f001"
