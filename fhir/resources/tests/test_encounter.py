...
def impl_encounter_1(inst):
    assert inst.actualPeriod.end == fhirtypes.DateTime.validate(
        "2015-01-17T16:30:00+10:00"
    )
    assert inst.actualPeriod.start == fhirtypes.DateTime.validate(
...
def impl_encounter_3(inst):
    ...
    assert (
        inst.admission.dischargeDisposition.coding[0].system == "http://snomed.info/sct"
    )
    assert (
