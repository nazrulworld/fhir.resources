...
def impl_episodeofcare_1(inst):
    ...
    assert inst.statusHistory[0].period.start == fhirtypes.DateTime.validate("2014-09-01")
    assert inst.statusHistory[0].status == "planned"
    assert inst.statusHistory[1].period.end == fhirtypes.DateTime.validate("2014-09-21")
    assert inst.statusHistory[1].period.start == fhirtypes.DateTime.validate("2014-09-15")
    assert inst.statusHistory[1].status == "active"
    assert inst.statusHistory[2].period.end == fhirtypes.DateTime.validate("2014-09-24")
    assert inst.statusHistory[2].period.start == fhirtypes.DateTime.validate("2014-09-22")
    assert inst.statusHistory[2].status == "onhold"
    assert inst.statusHistory[3].period.start == fhirtypes.DateTime.validate("2014-09-25")
    ...
    assert inst.type[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/episodeofcare-type"
...
def test_episodeofcare_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "episodeofcare-example.json"
    )
