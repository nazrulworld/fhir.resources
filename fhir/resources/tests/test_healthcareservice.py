...
def impl_healthcareservice_1(inst):
    ...
    assert inst.availabilityExceptions == "Reduced capacity is available during the Christmas period"
    ...
    assert inst.availableTime[1].availableStartTime == fhirtypes.Time.validate("08:30:00")
    ...
    assert inst.availableTime[2].availableStartTime == fhirtypes.Time.validate("09:30:00")
    ...
    assert inst.category[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/service-category"
    ...
    assert inst.notAvailable[0].during.start == fhirtypes.DateTime.validate("2015-12-25")
    ...
    assert inst.notAvailable[1].during.start == fhirtypes.DateTime.validate("2016-01-01")
    ...
    "http://terminology.hl7.org/CodeSystem/service-provision-"
    "conditions"
    )
...
def test_healthcareservice_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "healthcareservice-example.json"
    )
