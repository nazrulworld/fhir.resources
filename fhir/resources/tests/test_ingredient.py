...
def impl_ingredient_1(inst):
    ...
    assert inst.substance.code.concept.coding[0].system == "http://ema.europa.eu/example/substance"
    assert inst.substance.strength[0].presentationRatio.denominator.code == "{tablet}"
    assert inst.substance.strength[0].presentationRatio.denominator.system == "http://unitsofmeasure.org"
    assert inst.substance.strength[0].presentationRatio.denominator.unit == "tablet"
    assert float(inst.substance.strength[0].presentationRatio.denominator.value) == float(1)
    assert inst.substance.strength[0].presentationRatio.numerator.code == "mg"
    assert inst.substance.strength[0].presentationRatio.numerator.system == "http://unitsofmeasure.org"
    assert inst.substance.strength[0].presentationRatio.numerator.unit == "mg"
    assert float(inst.substance.strength[0].presentationRatio.numerator.value) == float(2.5)
...
def test_ingredient_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "ingredient-example.json"
    )
