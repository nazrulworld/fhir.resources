...
def impl_library_1(inst):
    ...
    assert inst.purpose == (
    "This library defines the Opioid Management Terminology "
...
def test_library_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "library-omtk-modelinfo.json"
    )
...
def test_library_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "library-hiv-indicators.json"
    )
    inst = library.Library.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_library_4(inst):
    ...
    assert inst.dataRequirement[0].codeFilter[0].valueSet == "http://example.org/fhir/ValueSet/benzodiazepines"
...
def impl_library_9(inst):
    ...
    assert inst.dataRequirement[1].codeFilter[2].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.102.12.1011"
    ...
    assert inst.dataRequirement[2].codeFilter[2].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.102.12.1012"
    ...
    assert inst.dataRequirement[3].codeFilter[2].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.101.12.1061"
    assert inst.dataRequirement[3].type == "Encounter"
    assert inst.dataRequirement[4].codeFilter[0].path == "diagnosis"
    assert inst.dataRequirement[4].codeFilter[0].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.198.12.1012"
    assert inst.dataRequirement[4].type == "DiagnosticReport"
    assert inst.dataRequirement[5].codeFilter[0].path == "code"
    assert inst.dataRequirement[5].codeFilter[0].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.196.12.1001"
    ...
    assert inst.dataRequirement[6].codeFilter[1].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.196.12.1001"
    ...
    assert inst.dataRequirement[7].codeFilter[1].valueSet == "urn:oid:2.16.840.1.113883.3.464.1003.196.12.1001"
    assert inst.dataRequirement[7].type == "MedicationStatement"
    assert inst.date == fhirtypes.DateTime.validate("2015-07-22")
    assert inst.description == (
    "Logic for CMS 146: Appropriate Testing for Children with "
    "Pharyngitis"
    )
    assert inst.experimental is True
...
def test_library_9(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "library-cms146-example.json"
    )
    inst = library.Library.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
