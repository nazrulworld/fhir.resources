...
def impl_media_1(inst):
    ...
    assert inst.modality.coding[0].system == "http://terminology.hl7.org/CodeSystem/media-modality"
...
def test_media_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "media-example.json"
    )
...
def impl_media_2(inst):
    ...
    "urn:oid:1.2.840.11361907579238403408700.3.1.04.1997032715003"
    "3"
    )
    ...
    assert inst.identifier[2].value == "urn:oid:1.2.840.113619.2.21.848.34082.0.538976288.3"
    assert inst.identifier[3].system == "urn:dicom:uid"
    assert inst.identifier[3].type.text == "seriesId"
    assert inst.identifier[3].value == "urn:oid:1.2.840.113619.2.21.3408.700.0.757923840.3.0"
    assert inst.meta.tag[0].code == "HTEST"
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.modality.coding[0].code == "US"
    assert inst.modality.coding[0].system == "http://dicom.nema.org/resources/ontology/DCM"
    assert inst.status == "completed"
...
def test_media_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "media-example-dicom.json"
    )
    inst = media.Media.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_media_3(inst):
    ...
    assert inst.basedOn[0].identifier.system == "http://someclinic.org/fhir/NamingSystem/imaging-orders"
...
def test_media_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "media-example-xray.json"
    )
    inst = media.Media.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_media_4(inst):
    ...
    assert inst.content.data == bytes_validator("dG9vIGJpZyB0b28gaW5jbHVkZSB0aGUgd2hvbGU=")
    assert inst.content.id == "a1"
...
def test_media_4(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "media-example-sound.json"
    )
    inst = media.Media.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
