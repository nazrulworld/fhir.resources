...
def impl_medicationrequest_1(inst):
    ...
    assert inst.medicationCodeableConcept.coding[0].display == "Proventil HFA 90mcg/actuat metered dose inhaler, 200 actuat"
...
def test_medicationrequest_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationrequest0326.json"
    )
...
def impl_medicationrequest_2(inst):
    ...
    assert inst.dosageInstruction[0].method.coding[0].display == "Instill - dosing instruction imperative (qualifier value)"
    ...
    assert inst.dosageInstruction[0].route.coding[0].display == "Ophthalmic route (qualifier value)"
...
def test_medicationrequest_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationrequest0330.json"
    )
    inst = medicationrequest.MedicationRequest.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_medicationrequest_3(inst):
    ...
    assert inst.dosageInstruction[0].asNeededCodeableConcept.coding[0].code == "32914008"
    assert inst.dosageInstruction[0].asNeededCodeableConcept.coding[0].display == "Restless Legs"
...
def test_medicationrequest_4(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationrequest0306.json"
    )
    inst = medicationrequest.MedicationRequest.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_medicationrequest_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationrequest0307.json"
    )
    inst = medicationrequest.MedicationRequest.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_medicationrequest_7(inst):
    ...
    assert inst.medicationCodeableConcept.coding[0].display == "Fentanyl 25micrograms/hour patch (product)"
...
def impl_medicationrequest_8(inst):
    ...
    assert float(inst.dosageInstruction[0].doseAndRate[0].rateRatio.denominator.value) == float(20)
    ...
    assert float(inst.dosageInstruction[0].doseAndRate[0].rateRatio.numerator.value) == float(1.8)
    assert inst.dosageInstruction[0].doseAndRate[0].type.coding[0].code == "ordered"
    assert inst.dosageInstruction[0].doseAndRate[0].type.coding[0].display == "Ordered"
    assert inst.dosageInstruction[0].doseAndRate[0].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/dose-rate-type"
    assert inst.dosageInstruction[0].maxDosePerLifetime.code == "mg"
    assert inst.dosageInstruction[0].maxDosePerLifetime.system == "http://unitsofmeasure.org"
    ...
    assert inst.dosageInstruction[0].sequence == 1
    assert inst.dosageInstruction[0].text == (
    "1.8 mg/kg IV infusion over 20 minutes every 3 weeks for 16 "
    "cycles"
    )
...
def test_medicationrequest_8(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationrequest0316.json"
    )
    inst = medicationrequest.MedicationRequest.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_medicationrequest_9(inst):
    ...
    assert inst.reasonCode[0].coding[0].display == "On subcutaneous insulin for diabetes mellitus (finding)"
    assert inst.reasonCode[0].coding[0].system == "http://snomed.info/sct"
    assert inst.requester.display == "Patrick Pump"
    assert inst.requester.reference == "Practitioner/f007"
    assert inst.status == "completed"
...
def impl_medicationrequest_10(inst):
    ...
    assert float(inst.dosageInstruction[0].doseAndRate[0].rateQuantity.value) == float(4)
    assert inst.dosageInstruction[0].doseAndRate[0].type.coding[0].code == "ordered"
    assert inst.dosageInstruction[0].doseAndRate[0].type.coding[0].display == "Ordered"
    assert inst.dosageInstruction[0].doseAndRate[0].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/dose-rate-type"
...
def test_medicationrequest_10(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationrequest0336.json"
    )
    inst = medicationrequest.MedicationRequest.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
