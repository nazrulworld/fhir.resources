...
def impl_packagedproductdefinition_1(inst):
    assert inst.description == "ALU-PVC/PVDC BLISTERS. CARTONS OF 10 FILM-COATED TABLETS."
    ...
    assert inst.package.material[0].coding[0].system == "http://ema.europa.eu/example/packageItemContainerMaterial"
    assert float(inst.package.package[0].containedItem[0].amount.value) == float(10)
    assert inst.package.package[0].containedItem[0].item.reference.reference == "ManufacturedItemDefinition/tablet"
    ...
    assert inst.package.package[0].material[0].coding[0].system == "http://ema.europa.eu/example/packageItemContainerMaterial"
    assert inst.package.package[0].material[1].coding[0].code == "PVDC"
    assert inst.package.package[0].material[1].coding[0].system == "http://ema.europa.eu/example/packageItemContainerMaterial"
    assert inst.package.package[0].material[2].coding[0].code == "alu"
    assert inst.package.package[0].material[2].coding[0].system == "http://ema.europa.eu/example/packageItemContainerMaterial"
    ...
    assert inst.package.package[0].property[0].valueQuantity.system == "http://unitsofmeasure.org"
    ...
    assert inst.package.package[0].property[1].valueQuantity.system == "http://unitsofmeasure.org"
    ...
    assert inst.package.package[0].shelfLifeStorage[0].periodDuration.system == "http://unitsofmeasure.org"
    assert inst.package.package[0].shelfLifeStorage[0].periodDuration.unit == "year"
    assert float(inst.package.package[0].shelfLifeStorage[0].periodDuration.value) == float(3)
    assert inst.package.package[0].shelfLifeStorage[0].specialPrecautionsForStorage[0].coding[0].code == "none"
    assert inst.package.package[0].shelfLifeStorage[0].specialPrecautionsForStorage[0].coding[0].system == "http://ema.europa.eu/example/specialprecautionsforstorage"
    assert inst.package.package[0].shelfLifeStorage[0].type.coding[0].code == "ShelfLifeofPackagedMedicinalProduct"
    assert inst.package.package[0].shelfLifeStorage[0].type.coding[0].system == "http://ema.europa.eu/example/shelfLifeTypePlaceHolder"
    assert inst.package.package[0].type.coding[0].code == "Blister"
    assert inst.package.package[0].type.coding[0].system == "http://ema.europa.eu/example/packageitemcontainertype"
    ...
    assert inst.package.type.coding[0].system == "http://ema.europa.eu/example/packageitemcontainertype"
...
def impl_packagedproductdefinition_2(inst):
    ...
    assert inst.packageFor[0].reference == "MedicinalProductDefinition/ProductThatHasThisPackType"
    assert inst.text.status == "generated"
...
def test_packagedproductdefinition_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "packagedproductdefinition-example-co-packaged-liquid-and-syringe.json"
    )
    inst = packagedproductdefinition.PackagedProductDefinition.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
