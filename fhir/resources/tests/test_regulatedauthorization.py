...
def test_regulatedauthorization_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "regulatedauthorization-example-basic-drug-auth.json"
    )
...
def impl_regulatedauthorization_2(inst):
    assert inst.case.application[0].dateDateTime == fhirtypes.DateTime.validate("2015-08-01")
    assert inst.case.application[0].identifier.system == "http://ema.europa.eu/example/applicationidentifier-number"
    ...
    assert inst.case.application[1].identifier.system == "http://ema.europa.eu/example/applicationidentifier-number"
    ...
    assert inst.case.identifier.system == "http://ema.europa.eu/example/procedureidentifier-number"
