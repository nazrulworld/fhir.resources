...
def impl_requestgroup_1(inst):
    ...
    assert inst.action[0].action[0].action[0].action[0].action[0].resource.reference == "#1111"
    ...
    assert inst.action[0].action[0].action[0].action[0].action[1].resource.reference == "#2222"
    ...
    assert inst.instantiatesCanonical[0] == "http://example.org/fhir/PlanDefinition/KDN5"
...
def test_requestgroup_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "requestgroup-kdn5-example.json"
    )
...
def impl_requestgroup_2(inst):
    ...
    assert inst.action[0].description == "Administer medications at the appropriate time"
    ...
    "Administer medication 1, followed an hour later by "
    "medication 2"
    )
    assert inst.action[0].timingDateTime == fhirtypes.DateTime.validate("2017-03-06T19:00:00Z")
...
def test_requestgroup_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "requestgroup-example.json"
    )
    inst = requestgroup.RequestGroup.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
