...
class RiskAssessmentTests(unittest.TestCase):
    ...
    def implRiskAssessment2(self, inst):
        ...
        self.assertEqual(
            ...
            force_bytes("risk-assessment-cardiac"),
        )
        self.assertEqual(force_bytes(inst.meta.tag[0].code), force_bytes("HTEST"))
        self.assertEqual(
            force_bytes(inst.meta.tag[0].display), force_bytes("test health data")
    ...
    def implRiskAssessment3(self, inst):
        ...
        self.assertEqual(
            force_bytes(inst.note[0].text), force_bytes("High degree of certainty")
        )
        self.assertEqual(
    ...
    def testRiskAssessment4(self):
        inst = self.instantiate_from("riskassessment-riskexample.json")
        self.assertIsNotNone(inst, "Must have instantiated a RiskAssessment instance")
    ...
    def implRiskAssessment4(self, inst):
        self.assertEqual(
            force_bytes(inst.code.coding[0].code), force_bytes("709510001")
        )
        self.assertEqual(
            force_bytes(inst.code.coding[0].display),
            force_bytes("Assessment of risk for disease (procedure)"),
        )
        self.assertEqual(
            force_bytes(inst.code.coding[0].system),
            force_bytes("http://browser.ihtsdotools.org/"),
        )
        self.assertEqual(force_bytes(inst.id), force_bytes("riskexample"))
        self.assertEqual(
            force_bytes(inst.identifier[0].system), force_bytes("http://example.org")
        )
        self.assertEqual(force_bytes(inst.identifier[0].use), force_bytes("official"))
        self.assertEqual(
            force_bytes(inst.identifier[0].value),
            force_bytes("risk-assessment-breastcancer1"),
        )
        self.assertEqual(force_bytes(inst.meta.tag[0].code), force_bytes("HTEST"))
        self.assertEqual(
            force_bytes(inst.meta.tag[0].display), force_bytes("test health data")
        ...
        self.assertEqual(
            ...
            force_bytes("This risk assessment is for reference only"),
        )
        self.assertEqual(inst.occurrenceDateTime.date, FHIRDate("2017-10-10").date)
        self.assertEqual(inst.occurrenceDateTime.as_json(), "2017-10-10")
        self.assertEqual(
            force_bytes(inst.prediction[0].outcome.text), force_bytes("Breast Cancer")
        )
        self.assertEqual(inst.prediction[0].probabilityDecimal, 0.000368)
        self.assertEqual(
            force_bytes(inst.prediction[0].whenRange.high.code), force_bytes("a")
        )
        self.assertEqual(
        ...
        self.assertEqual(inst.prediction[0].whenRange.high.value, 57)
        self.assertEqual(
            force_bytes(inst.prediction[0].whenRange.low.code), force_bytes("a")
        )
        self.assertEqual(
        ...
        self.assertEqual(inst.prediction[0].whenRange.low.value, 54)
        self.assertEqual(force_bytes(inst.status), force_bytes("final"))
        self.assertEqual(
    ...
    def implRiskAssessment5(self, inst):
        ...
        self.assertEqual(
            ...
            force_bytes("Unknown risk of developing breast cancer"),
        )
        self.assertEqual(force_bytes(inst.status), force_bytes("final"))
        self.assertEqual(force_bytes(inst.text.status), force_bytes("additional"))

    def testRiskAssessment6(self):
        inst = self.instantiate_from("riskassessment-example-prognosis.json")
        self.assertIsNotNone(inst, "Must have instantiated a RiskAssessment instance")
        self.implRiskAssessment6(inst)

        js = inst.as_json()
        self.assertEqual("RiskAssessment", js["resourceType"])
        inst2 = riskassessment.RiskAssessment(js)
        self.implRiskAssessment6(inst2)

    def implRiskAssessment6(self, inst):
        ...
        self.assertEqual(
            ...
            force_bytes("http://terminology.hl7.org/CodeSystem/risk-probability"),
        )
        self.assertEqual(force_bytes(inst.status), force_bytes("final"))
        self.assertEqual(force_bytes(inst.text.status), force_bytes("additional"))
