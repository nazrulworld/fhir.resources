...
def impl_schedule_1(inst):
    ...
    assert inst.planningHorizon.end == fhirtypes.DateTime.validate("2017-12-25T09:30:00Z")
    assert inst.planningHorizon.start == fhirtypes.DateTime.validate("2017-12-25T09:15:00Z")
...
def test_schedule_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "schedule-example.json"
    )
    inst = schedule.Schedule.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
