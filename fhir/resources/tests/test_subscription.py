...
def impl_subscription_1(inst):
    assert inst.channel.endpoint == "https://biliwatch.com/customers/mount-auburn-miu/on-result"
...
def test_subscription_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "subscription-example-error.json"
    )
...
def test_subscription_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "subscription-example.json"
    )
    inst = subscription.Subscription.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
