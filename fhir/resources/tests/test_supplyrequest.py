...
class SupplyRequestTests(unittest.TestCase):
    ...
    def implSupplyRequest1(self, inst):
        ...
        self.assertEqual(
            force_bytes(inst.identifier[0].value), force_bytes("Order10284")
        )
        self.assertEqual(
            force_bytes(inst.itemCodeableConcept.coding[0].code),
            force_bytes("BlueTubes"),
        )
        self.assertEqual(
            force_bytes(inst.itemCodeableConcept.coding[0].display),
        ...
        self.assertEqual(
            force_bytes(inst.reasonCode[0].coding[0].code), force_bytes("stock_low")
        )
        self.assertEqual(
