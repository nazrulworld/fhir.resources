...
def impl_task_1(inst):
    ...
    assert inst.executionPeriod.end == fhirtypes.DateTime.validate("2016-10-31T18:45:05+10:00")
    assert inst.executionPeriod.start == fhirtypes.DateTime.validate("2016-10-31T08:25:05+10:00")
    ...
    assert inst.performerType[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/task-performer-type"
    ...
    assert inst.restriction.period.end == fhirtypes.DateTime.validate("2016-11-02T09:45:05+10:00")
...
def test_task_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example6.json"
    )
...
def impl_task_2(inst):
    ...
    assert inst.code.coding[0].system == "http://terminology.hl7.org/CodeSystem/financialtaskcode"
    ...
    assert inst.input[0].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/financialtaskinputtype"
    assert inst.input[0].valueCode == "ClaimResponse"
    assert inst.input[1].type.coding[0].code == "period"
    assert inst.input[1].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/financialtaskinputtype"
...
def test_task_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example-fm-poll.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_task_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example1.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_task_4(inst):
    ...
    assert inst.input[0].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/financialtaskinputtype"
    assert inst.input[0].valueReference.identifier.system == "http://nationalinsurers.com/claimresponse"
    ...
    assert inst.input[2].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/financialtaskinputtype"
    assert inst.input[2].valuePositiveInt == 2
    assert inst.input[3].type.coding[0].code == "item"
    assert inst.input[3].type.coding[0].system == "http://terminology.hl7.org/CodeSystem/financialtaskinputtype"
...
def test_task_4(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example-fm-reprocess.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_task_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example3.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_task_6(inst):
    ...
    assert inst.output[0].type.coding[0].system == "http://hl7.org/financial-taskoutputtype"
...
def test_task_6(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example-fm-status-resp.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_task_7(inst):
    ...
    assert inst.encounter.display == "Example In-Patient Encounter"
    assert inst.encounter.reference == "Encounter/example"
    assert inst.executionPeriod.start == fhirtypes.DateTime.validate("2016-10-31T08:45:05+10:00")
    ...
    assert inst.requester.reference == "Organization/1832473e-2fe0-452d-abe9-3cdb9879522f"
    assert inst.restriction.period.end == fhirtypes.DateTime.validate("2016-11-01T09:45:05+10:00")
    assert inst.restriction.repetitions == 1
...
def test_task_7(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example2.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_task_8(inst):
    ...
    assert inst.identifier[0].value == "20181012-001"
    assert inst.input[0].type.coding[0].code == "origresponse"
    assert inst.input[0].type.coding[0].system == "http://hl7.org/financial-taskinputtype"
    assert inst.input[0].valueReference.identifier.system == "http://nationalinsurers.com/claimresponse"
    assert inst.input[0].valueReference.identifier.value == "CR201810040001234"
    assert inst.intent == "order"
    assert inst.lastModified == fhirtypes.DateTime.validate("2018-10-04T08:25:05+10:00")
...
def test_task_8(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example-fm-release.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_task_9(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example-fm-cancel.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_task_10(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example5.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
